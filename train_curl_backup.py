...
def main():
    ...
    if (config.checkpoint_filepath is not None) and (config.inference_img_dirpath is not None):

        '''
        config.inference_img_dirpath: the actual filepath should have "input" in the name an in the level above where the images 
        ...
        inference_data_loader = Adobe5kDataLoader(data_dirpath=config.inference_img_dirpath,
                                                img_ids_filepath=config.inference_img_dirpath+"/images_inference.txt")
        ...
        inference_data_loader = torch.utils.data.DataLoader(inference_dataset, batch_size=config.batch_size, shuffle=False,
                                                            num_workers=10)

        '''
        Performs inference on all the images in config.inference_img_dirpath
        '''
        logging.info(
            "Performing inference with images in directory: " + config.inference_img_dirpath)

        net = model.CURLNet()
        checkpoint = torch.load(config.checkpoint_filepath, map_location='cuda')
        ...
        if (config.checkpoint_filepath is not None) and (config.inference_img_dirpath is None):
                ...
                if valid_psnr > best_valid_psnr:
                    ...
                    logging.info(
                        "Validation PSNR has increased. Saving the more accurate model to file: " + 'curl_validpsnr_{}_validloss_{}_testpsnr_{}_testloss_{}_epoch_{}_model.pt'.format(valid_psnr,
                                                                                                                                                                                    valid_loss.tolist()[0], test_psnr, test_loss.tolist()[0],
                                                                                                                                                                                    epoch))
